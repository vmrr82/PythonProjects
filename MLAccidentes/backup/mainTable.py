...
file = pd.DataFrame(resultSet).sort_values(by=['fecha'])
...
app.layout = html.Div(lang='es',
    children =[html.Div([
        dbc.Row([html.Div([
            html.H1("VISOR DE ACCIDENTES DE CALVIÀ",className='header',style={'color':'#fff','font-family':'monospace','font-size':'4em'}),
        ...
        dbc.Row([html.Div([
                    ...
                    html.P(str(file['gid'].max()),className='numTotal')],className='bloqTotal'),
                html.Div([
                    html.H3('PRIMERO REGISTRADO',className='tituloPrimero'),
                    html.P(file['fecha'].min().strftime('%d-%m-%Y'),className='numFecha')],className='bloqPrimero'),
                html.Div([
                    html.H3('ÚLTIMO REGISTRADO',className='tituloUltimo'),
                    html.P(file['fecha'].max().strftime('%d-%m-%Y'),className='numFecha')],className='bloqUltimo'),
                html.Div([
        ...
        dbc.Row([html.Div([
            dbc.Col(html.Div([
                dcc.RadioItems(id='radioItem', options=["Puntos de colisión","Heatmap"],value="Puntos de colisión",inline=True)],className='radioItem')),
            dbc.Col(html.Div(dcc.Graph(id='mapaCallback',config={'displaylogo':False,'displayModeBar':False,'showLink':False},
                                    )),className='mapa')]),

            dbc.Col([html.Div([dash_table.DataTable(
                                    columns=[{  'name':'AÑO','id': 'aÑo','type':'numeric','selectable':True,'hideable':False},{
                                    ...
                                    style_cell_conditional=[{'if':{'column_id':'aÑo'},'width':'55px'},{'if':{'column_id':'mes'},'width':'90px'},
                                                            {'if':{'column_id':'dia_semana'},'width':'90px'},{'if':{'column_id':'gravedad'},'width':'100px'},
...
def update_graphs(filter_table,selected_columns,radioItem):
    ...
    if c[0].triggered: #filter datatble
        fig1 = px.scatter_mapbox(dff,lat='coord_y',lon='coord_x',
                                color='gravedad',
                                zoom=12, 
                                height=500,width=750,
                                category_orders={'gravedad':['SIN HERIDOS','HERIDO_HOSP-24','HERIDO_HOSP+24','FALLECIDO']},
                                mapbox_style=layer,
                                hover_data=['fecha','hora','ubicacion','n','ubicacion_','nucleo','gravedad','alcoholemia'],
...
@app.callback(Output('mainTable','filter_query'),
