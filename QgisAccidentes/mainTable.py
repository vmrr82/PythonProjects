...
app.layout = html.Div(lang='es',style={'width':'1500px','height':'864px'},
    children =[html.Div([
        dbc.Row([
            html.Div([
                    html.H3("TOTAL ACCIDENTES",className='tituloTotal'),
                    html.P(file['id'].count(),className='numTotal', title='Número total de accidentes desde el 01/01/2020')],style={'width':'220px','height':'90px'},className='bloqTotal'),
            html.Div([
                    ...
                    html.P(pd.to_datetime(file['fecha']).min().strftime('%d-%m-%Y'),className='numFecha',title='Primer accidente registrado')],
                            style={'width':'260px','height':'90px'},className='bloqPrimero'),
            html.Div([
                    ...
                    html.P(pd.to_datetime(file['fecha']).max().strftime('%d-%m-%Y'),className='numFecha',title='Último accidente registrado')],
                            style={'width':'240px','height':'90px'},className='bloqUltimo'),
            html.Div([
                    html.H3('MÁXIMO DIARIO',className='tituloMaxDiario'),
                    html.P(file['fecha'].value_counts().max(),className='numeroMaxDiario')],
                                style={'width':'210px','height':'90px'},className='bloqDiario',
                                    title='Máximo de accidentes diarios'),
            html.Div([
                    html.H3('MÁXIMO IMPLICADOS',className='tituloMaxImplicados'),
                    html.P(file['n_vehÍc'].max(),className='numeroMaxImplicados', title='Máximo número de implicados de un accidente')],
                        style={'width':'240px','height':'90px'},className='bloqImplicados')],className='top'),
            html.Img(src=r"assets/Policia_Local_Derecha.png",width='300px',title='Ajuntament de Calvià - Policía Local',className="bloqIcon"),
                    #html.Button(className='contact-button',title='atestados@calvia.com')],className='top'),
            ...
            html.Div([
                ...
                dbc.Col(
                    html.Div([dcc.Graph(id='mapaCallback',responsive=True,config={'displaylogo':False,'displayModeBar':True,'showLink':False},
                            style={'width':'900px','height':'450px'})]),className='mapa'),
                    html.Div(dcc.Graph(figure={'layout':{'paper_bgcolor':'rgb(8, 34, 85)','plot_bgcolor':'rgb(8, 34, 85)','font':{'color':'white'}}},id='figraphs',responsive=True,
                       config={'displaylogo':False,'displayModeBar':True,'showLink':False},style={'width':'500px','height':'450px'}),className='figGraphs')])]),
        dbc.Row([dbc.Col(
            children=[
                html.Div(dash_table.DataTable(
                                    columns=[{  'name':'ID','id': 'id','type':'numeric','selectable':False,'hideable':True},{
                                                ...
                                                'name':'Nº IMPLICADOS.','id':'n_vehÍc','type':'text','selectable':True,'hideable':True},{
                                    ...
                                    style_cell_conditional=[{'if':{'column_id':'id'},'width':'40px'},
                                                            ...
                                                            {'if':{'column_id':'ubicacion'},'width':'110px'},
                                                            {'if':{'column_id':'dia semana'},'width':'100px'},
                                                            {'if':{'column_id':'gravedad'},'width':'90px'},
                                                            {'if':{'column_id':'nucleo'},'width':'140px'},
                                                            {'if':{'column_id':'n_vehÍc'},'width':'90px'},
                                                            {'if':{'column_id':'alcoholemia'},'width':'90px'},
                ...
                html.Div(dcc.Graph(figure={'layout':{'paper_bgcolor':'rgb(8, 34, 85)','plot_bgcolor':'rgb(8, 34, 85)','font':{'color':'white'}}},id='secondFigGraphs',
                        responsive=True,config={'displaylogo':False,'displayModeBar':True,'showLink':False},style={'width':'500px','height':'450px'}),className='secondFigGraphs')]),               
                html.Div(id='totalFilter',className='totalFilter'),
                html.Div([html.Button(id='clear_filter',children='Limpiar Formulario',n_clicks=0,title='Limpiar Formulario')],id='filterButton')])
                ])])
...
#pie
    ...
    pie.update_layout(title_text=f'RESULTADOS TOTALES DE: {selected_columns[0]}'.upper(),title_font=dict(size=20, color='#fff', family="Lato, sans-serif"),xaxis_tickangle=0, plot_bgcolor='rgb(8, 34, 85)', paper_bgcolor='rgb(8, 34, 85)',font=dict(color='#8a8d93'))
...
#hist
    ...
    hist.update_layout(margin=dict(t=90, b=0, l=70, r=40),
                            xaxis_tickangle=45,
